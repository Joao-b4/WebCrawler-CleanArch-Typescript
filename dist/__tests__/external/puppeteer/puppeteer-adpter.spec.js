...
const makeSut = () => {
    const sut = new puppeteer_adapter_1.PuppeteerAdapter(page_function_mock_1.PageFunctionMock);
    return {
        sut
    };
};
describe('PuppeteerAdapter Browser', () => {
    ...
    test('that it should return ScrapingResult', async () => {
        jest.spyOn(puppeteer_mock_1.stubPage, 'evaluate').mockReturnValue(Promise.resolve(page_function_mock_1.ScrapingResultMock));
